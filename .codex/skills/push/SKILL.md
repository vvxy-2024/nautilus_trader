...
7. Validate PR body content against the PR template sections and ensure no
...
# Minimal validation gate (pick commands for current scope)
...
if rg -n "<!--" "$tmp_pr_body"; then
  echo "PR body still contains unresolved template placeholders." >&2
  rm -f "$tmp_pr_body"
  exit 1
fi
rm -f "$tmp_pr_body"
